...
# We want know if: smoker temp. decreases more than 15 degrees F in 2.5 min; trigger smoker alert!

smoker_alert_limit = 15 # if temp decreases by this, send a smoker alert

#######################################################################################
## define delete_queue
def delete_queue(host: str, queue_name: str):
    """
    Delete queues each time program is run to clear old messages.
    """
...
def smoker_callback(ch, method, properties, body):
    ...
    # acknowledge message was received and processed 
    ...
    smoker_deque.append(message)

    # identifying first item in the deque
    smoker_deque_temp = smoker_deque[0]
   
    # splitting date & timestamp from temp in column
    # will now have date and timestamp with index 0 and temp with index 1
    # this will be looking 2.5mins prior
...
def main(hn: str, qn: str):
    ...
    # when a statement can go wrong, use try-except block
    try:
    ...
    try:
        ...
        channel.basic_consume( queue=smoker_queue, on_message_callback=smoker_callback)
