...
food_a_deque = deque(maxlen=20) # limited to 20 items (the 20 most recent readings)
...
def food_A_callback(ch, method, properties, body):
    """ Define behavior on getting a message about the food A temperature."""
    ...
    food_a_deque.append(message)

    # identifying first item in the deque
    food_a_deque_temp = food_a_deque[0]
    ...
    # defining current food A temperature
    ...
    food_a_change = round(food_a_now_temp - food_a_temp_1, 1)
    # defining smoker alert
    if food_a_change >= food_stall_alert_limit:
        print(f" FOOD STALL!! The temperature of the food has changed by 1 degree or less in 10 min (or 20 readings). \n          foodA temp change = {food_a_change} degrees F = {food_a_now_temp} - {food_a_temp_1}")
...
def main(hn: str, qn: str):
    ...
    try:
        ...
        # use the channel to declare durable queue
        # durable queue will survive a RabbitMQ server restart
        ...
        channel.basic_consume( queue=food_a_queue, on_message_callback=food_A_callback)
